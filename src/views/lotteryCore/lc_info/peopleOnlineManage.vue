    ...
    <el-dialog :visible.sync="addPercentDialog" title="新增" width="410px" :top="dialogMt" :close-on-click-modal="false">
      <div class="dialog-box">
        <p>
          ...
          <el-input style="width:200px;" v-model="add_percent.visonName"/>
        </p>
        <p>
          <span class="dialog-label-item visonstate-item-span">
            <span class="required"></span>在线人数版本状态：
          </span>
          <el-input style="width:200px;" v-model="add_percent.plamPercentVisonState" />
        </p>
        <p>
          <span class="dialog-label-item addPercent-item-span">
      ...
      <div class="dialog-box">
        ...
        >
          ...
          <el-table-column prop="onlineVer" align="center" label="选择二级彩种在线人数版本号"   show-overflow-tooltip>
            <template slot-scope="scope">
              <el-select  style="background:#00ffff" placeholder="请选择" v-if="editNames == '编辑'"  value-key="value" v-model="scope.row.onlineVer" @change="verChange(scope.row,scope.row.quotaVer,scope.$index)">
                <el-option v-for="(item,index) in scope.row.detailList" :key="index" :label="item.onlineVer" :value="item.lotteryOnlineVerId"></el-option>
              </el-select>
    ...
    <el-dialog :visible.sync="addPercentMarkDialog" title="编辑说明" width="400px" :top="dialogMt" :close-on-click-modal="false">
      <div class="dialog-box">
        <p>
          ...
          <el-input style="width: 200px;" v-model="add_percentMark.visonName"/>
        </p>
        <p>
          <span class="dialog-label-item online-people">
            <span class="required">*</span>版本说明：
  ...
  created() {
    ...
    if(this.$store.getters.alreadyClose.includes(this.$store.getters.currentView)){
      if(this.$store.getters.currentView == this.$route.name) {
        ...
        this.$store.state.lc_peopleOnline.size = 10;
        this.$store.state.lc_peopleOnline.disableArr =[{value:"",name:"全部"}] 
